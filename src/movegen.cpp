...
static void AddWhitePawnCapMov(const s_Board *pos, const int from, const int to, const int cap, s_MOVELIST *list)
{
...
static void AddWhitePawnMov(const s_Board *pos, const int from, const int to, s_MOVELIST *list)
{
    ASSERT(SqOnBoard(from));
    ASSERT(SqOnBoard(to));
...
static void AddBlackPawnCapMov(const s_Board *pos, const int from, const int to, const int cap, s_MOVELIST *list)
{
    ASSERT(PieceValidEmpty(cap));
    ASSERT(SqOnBoard(from));
    ASSERT(SqOnBoard(to));
...
static void AddBlackPawnMov(const s_Board *pos, const int from, const int to, s_MOVELIST *list)
{
    ASSERT(SqOnBoard(from));
    ASSERT(SqOnBoard(to));
...
void GenerateAllMoves(const s_Board *pos, s_MOVELIST *list)
{
    ...
    if (side == WHITE)
    {
        ...
        for (pceNum = 0; pceNum < pos->pceNum[wP]; pceNum++)
        {
            ...
            {
                AddWhitePawnMov(pos, sq, sq + 10, list);
    ...
    else
    {
        for (pceNum = 0; pceNum < pos->pceNum[bP]; pceNum++)
        {
            ...
            if (pos->pieces[sq - 10] == EMPTY)
            {
                AddBlackPawnMov(pos, sq, sq - 10, list);
            ...
            {
                AddBlackPawnCapMov(pos, sq, sq - 9, pos->pieces[sq - 9], list);
            }
    ...
    pce = LoopSlidePce[pceIndex++]; // Post Increment
    ...
    while (pce != 0)
    {
        ...
        for (pceNum = 0; pceNum < pos->pceNum[pce]; pceNum++)
        {
            ...
            for (index = 0; index < numDir[pce]; index++)
            {
                ...
                {
                    //(Black) 1 ^ 1 == 0 (White)     or     (White) 0 ^ 1 == 1  (Black)
    ...
    pce = LoopNonSlidePce[pceIndex++]; // Post Increment
